...
// Linux). However, in this example, we install a TbfQueueDisc with a capacity
// of 10000 packets.
//
// Network topology
//
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("rate", "Speed of tokens arriving in first bucket in bps or Bps", rate);
  cmd.AddValue ("peakRate", "Speed of tokens arriving in second bucket in bps or Bps", peakRate);
  ...
  pointToPoint.SetQueue ("ns3::DropTailQueue", "Mode", StringValue ("QUEUE_MODE_BYTES"), "MaxBytes", UintegerValue (6553500));
  ...
  uint16_t handle = tch.SetRootQueueDisc ("ns3::TbfQueueDisc",
                                          ...
                                          "QueueLimit", UintegerValue (6553500),
                                          "Burst", UintegerValue (burst),
                                          "Mtu", UintegerValue (mtu),
                                          "Rate", DataRateValue (DataRate (rate)),
                                          "PeakRate", DataRateValue (DataRate (peakRate)));
  // Add the internal queue used by Tbf
  tch.AddInternalQueues (handle, 1, "ns3::DropTailQueue", "Mode", StringValue ("QUEUE_MODE_BYTES"), "MaxBytes", UintegerValue (6553500));
