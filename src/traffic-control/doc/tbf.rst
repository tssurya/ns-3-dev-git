...
TBF is a simple classless qdisc that allows controlling the bandwidth of the throughput according
...
2. Data rate < Token rate : Very less number of tokens are depleted.
  * Burst Condition : So the tokens might accumulate and the bucket might become
    ...
    MTU. This second bucket cannot store large amount of tokens, but it’s
    replenishing rate will be a lot faster than the one of the big bucket. This second
    rate is called 'peakRate' and it will determine the maximum speed of a burst.
...
The TBF queue disc does not require packet filters, does not admit
...
as the queue disc and having a size equal to the TBF QueueLimit attribute
...
If the Mtu and PeakRate are zero, then the second bucket does not exist. In order to activate
the second bucket, both the Mtu and PeakRate values have to be greater than zero. If 
the Mtu value is zero and not set by the user, then if a NetDevice exits, the Mtu's 
...
* class :cpp:class:`TbfQueueDisc`: This class implements the main TBF algorithm:
  ...
  * ``TbfQueueDisc::DoDequeue ()``: This routine performs the dequeuing of packets according to the following logic:
    ...
    * It calls ``TbfQueueDisc::DoPeek ()`` and calculates 'packetByteSize' the size of the packet to be dequeued in bytes.
    ...
    * From this second bucket token number, packetByteSize is subtracted.
    * Now the number of tokens in the first bucket are updated according to 'Rate' and 'delta'.
    * From this first bucket token number, packetByteSize is subtracted.
...
* ``Mode:`` TBF operating mode (BYTES or PACKETS). The default mode is BYTES.
* ``QueueLimit:`` The maximum number of packets the queue can hold. The default value is 125000 bytes.
* ``Burst:`` Size of the first bucket, in bytes. The default value is 125000 bytes.
* ``Mtu:`` Size of second bucket defaults to a single packet size if there is a netDevice and 0bytes otherwise.
* ``Rate:`` Speed at which tokens enter the first bucket in B/s or bps. The default value is 125KB/s.
* ``PeakRate:`` Speed at which tokens enter the second bucket in B/s or bps. The default value is 0KB/s.
