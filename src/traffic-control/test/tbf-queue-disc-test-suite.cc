...
public:
   ...
   */
  TbfQueueDiscTestItem (Ptr<Packet> p, const Address & addr, uint16_t protocol);
...
TbfQueueDiscTestItem::TbfQueueDiscTestItem (Ptr<Packet> p, const Address & addr, uint16_t protocol)
...
private:
   ...
   * \param mode the mode
   */
  void RunTbfTest (StringValue mode);
};
...
void
TbfQueueDiscTestCase::RunTbfTest (StringValue mode)
{
  ...
  /* 1. There is no second bucket since "peakRate" and "mtu" are set to 0.
     2. A simple enqueue of five packets, each containing 1500B is followed by the dequeue those five packets.
     3. The subtraction of tokens from the first bucket to send out each of the five packets is monitored and verified.
     Note : The number of tokens in the first bucket is full at the beginning. With the dequeuing of each packet, the 
            number of tokens keeps decreasing. So packets are dequeued as long as there are enough tokens in the bucket. */ 
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("Mode", mode), true,
                         "Verify that we can actually set the attribute Mode");

  if (queue->GetMode () == TbfQueueDisc::QUEUE_DISC_MODE_BYTES)
    {
      modeSize = pktSize;
  ...
  /* This test checks the burst control ability of TBF. 10 packets each of size 1000 bytes are enqueued followed by
     their dequeue. Although the Token rate is pretty high when compared to the data rate, all the packets (except 
     for the first packet) still get blocked and waking of queue get scheduled. Hence when adequate tokens are present
     in the second bucket, the packets get transmitted. So basically the transmission of packets falls under the 
...
{
  queue->Enqueue (Create<TbfQueueDiscTestItem> (Create<Packet> (size), dest, 0));
}
...
{
  RunTbfTest (StringValue ("QUEUE_DISC_MODE_PACKETS"));
  RunTbfTest (StringValue ("QUEUE_DISC_MODE_BYTES"));
  Simulator::Destroy ();
