...
public:
  ...
  /**
    * \brief Set the speed of the tokens entering the first bucket in bps or Bps.
    *
    * \param rate The speed of first bucket tokens in bps or Bps.
    */
  void SetRate (DataRate rate);

  /**
    * \brief Get the speed of the tokens entering the first bucket in bps or Bps.
    *
    * \returns The speed of first bucket tokens in bps or Bps.
    */
  DataRate GetRate (void) const;

  /**
    * \brief Set the speed of the tokens entering the second bucket in bps or Bps.
    *
    * \param peakRate The speed of second bucket tokens in bps or Bps.
    */
  void SetPeakRate (DataRate peakRate);

  /**
    * \brief Get the speed of the tokens entering the second bucket in bps or Bps.
    *
    * \returns The speed of second bucket tokens in bps or Bps.
    */
...
private:
  ...
  uint32_t m_limit;      //!< Maximum number of packets/bytes that can be stored in the queue.
  uint32_t m_burst;      //!< Size of first bucket in bytes.
  uint32_t m_mtu;        //!< Size of second bucket in bytes.
  DataRate m_rate;       //!< Speed at which tokens enter the first bucket.
  DataRate m_peakRate;   //!< Speed at which tokens enter the second bucket.
